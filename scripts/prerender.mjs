...
// External hosts that must not block prerendering or pollute the snapshot
const BLOCKED_HOSTS = [
...
async function main() {
  ...
  try {
    ...
    const context = await browser.newContext({ userAgent: 'MobiWavePrerender/1.0 (+https://mobiwave.co.ke)' })
    ...
    for (const route of routes) {
      ...
      try {
        ...
        // SEOHead's schema node is only created after the route effect runs.
        // Use state:'attached' - script/meta elements are never "visible".
        await page.waitForSelector('script#mw-seo-schema', { state: 'attached', timeout: 30000 })
        // Give effects (canonical, meta, schema text) a moment to settle
        await page.waitForTimeout(800)
        // Allow GSAP/scroll-triggered content to settle into final visible state
